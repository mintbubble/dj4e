...
class AdListView(OwnerListView):
    ...
    def get(self, request):
        ...
        if strval:
            ...
            post_list = Ad.objects.filter(query).select_related().order_by('-updated_at')[:10]
        else:
            post_list = Ad.objects.all().order_by('-updated_at')[:10]

        # Augment the post_list
        for obj in post_list:
            obj.natural_updated = naturaltime(obj.updated_at)

        ctx = {'ad_list': post_list, 'search': strval}
...
# class SearchResultsView(OwnerListView):
#     model = Ad
#     template_name = "ads/ad_detail.html"
#
#     def get(self, request, pk):
